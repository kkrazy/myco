...
3. **Task-list etiquette — `/task`, `/skip`, `/cancel`, and stale-task reminders.** The user can intervene in your internal TaskList from the chat pane via three slash commands. The server (see `server/src/slashcmds.js handleTaskList` + `handleTaskSkip`) forwards them to your running session as a natural-language directive:
   ...
   - **fr-87 task↔item registry (per-item `/task` filtering).** When `/task` is typed inside a per-item chat panel, the server reads `_myco_/task-items.json` directly and replies WITHOUT going through you — but only if the registry has entries for that item. To keep the registry populated, **every TaskCreate you fire during a `[chat:plan#<id>]` or `[run:plan#<id>]` turn must be followed by a `mcp__myco__register_task_item` call with `{ taskId, itemId: "<id>", subject, status: "pending" }`**. Same after each `TaskUpdate` that changes status — re-call `register_task_item` with the updated status so the registry filter (`pending` + `in_progress` only) stays accurate. The tool is idempotent (re-registering preserves `createdAt`). Skip the registry call for tasks that aren't scoped to a specific plan item (general housekeeping work spun from a bare chat-pane request); those still show up under `/task all`.
...
3. **Worktree-isolated subagent dispatch on queued items** (fr-90 design, kkrazy 2026-05-24). When a queue dispatch arrives with a `[wt:<path>#<branch>]` marker in the inbound user message, you are running against an **isolated git worktree** at `<path>` on branch `<branch>`. **Spawn a Task-tool subagent** for the actual work — pass the worktree path explicitly in the subagent's prompt (`Work in '<absolute-wt-path>'. Use bash commands with that as your cwd…`) so its file edits land in the isolated checkout, not the main repo. The subagent commits its changes to the branch + reports back. You (the parent) then either: leave the branch for human review (default — the plan card surfaces a `🔀 wt-<itemId>` chip the user clicks to diff/merge), or, if the item's `meta.autoMerge: true`, attempt a fast-forward merge to main yourself via `git merge --ff-only`. Skip the subagent indirection only if the work is trivially file-free (an analysis comment, a vote, a plan-item-status flip — those can run inline in the main agent).

   **Parallel-item subagent dispatch (fr-90 Phase 2).** The run-queue now dispatches up to `rec.runQueueMaxConcurrent` items concurrently (default 3). When you're about to spawn the worktree-subagent for a long-running item AND you can see (or expect) other in-flight queue work, **pass `run_in_background: true` to the Task call**. This frees your main-agent turn to handle the next queue dispatch instead of blocking on the subagent's completion — that's how parallelism actually materializes. The harness notifies you when the background subagent completes; you then stamp the matching `runs[]` + post the closeout comment via `mcp__myco__add_comment`. Skip `run_in_background` for short item work where blocking is fine (defaults to foreground; foreground returns the result inline). **Why**: SDK queries are serialized per session — only Task subagents can run truly in parallel, and only when explicitly backgrounded.

4. **Slash commands should be implemented as MCP tools whenever possible** (fr-88 design principle, kkrazy 2026-05-24). When adding a new operation that touches plan items / artifacts / session state, expose it FIRST as an `mcp__myco__*` tool in `server/src/myco-mcp.js` (or a sibling MCP server) with a typed zod schema. Then write the slash command as a thin wrapper that delegates to the same backend code path the tool invokes. Two surfaces, one source of truth: users type `/cmd`, the agent invokes the tool directly when it wants the same effect. fr-87's `register_task_item` tool + `/task` scoped read is the canonical example: tool registers the data, slash command reads + formats. Out of scope: auth/permission commands (`/admin`, `/setpat`, `/strict`) stay slashcmd-only — they're user-trust boundaries the agent shouldn't cross. Test cadence: each migration ships its own fr-87-shape test (tool-defined static guards + slashcmd-delegates pin + behavior simulation against a fake session).

   **Closeout-comment etiquette (mcp__myco__add_comment).** After shipping work on a plan item (commit pushed + tests green), call `mcp__myco__add_comment({itemId, text: '<closeout summary>'})` to leave a markdown summary on the item — commit ref, files touched, tests, future notes. This replaces the older pattern of writing a one-shot node script to mutate `_myco_/plan.json` directly: same data lands in the same place, but the tool path is server-validated (length cap, item exists, broadcast to attached clients) and shows up live in everyone's plan view. Use the tool whenever you'd otherwise write a `pushComment(itemId, …)` script. Skip the tool only when the user explicitly types `/comment` themselves — that's their voice, not yours.
